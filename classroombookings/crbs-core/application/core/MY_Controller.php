...
class MY_Controller extends CI_Controller
{
	...
	public function debug_language() {
		$site_language = $this->session->userdata('site_language') ?? 'english';
		echo 'Current session language: ' . $site_language;
	}
	...
	/**
     * Private function to handle language loading
     */
    private function load_language() {

		$lang = $this->session->userdata('site_language') ?? 'english';
        $current_controller = $this->router->fetch_class();
        $language_file = APPPATH . "language/{$lang}/{$current_controller}_lang.php";

        $this->lang->load('crbs', $lang);
		$this->lang->load('calendar', $lang);

        // Conditionally load the controller-specific language file
        if (file_exists($language_file)) {
            $this->lang->load($current_controller, $lang);
       }
