...
class Calendar
{
	...
	public static function get_localized_day_names($style = 'long') {
		$ci = get_instance();
		$day_names = self::get_day_names($style);
	
		$localized_names = [];
		foreach ($day_names as $key => $day) {
			// Use language keys for localization; fallback to original if translation is missing
			$localized_names[$key] = $ci->lang->line('calendar_day_' . strtolower($day)) ?: $day;
		}
