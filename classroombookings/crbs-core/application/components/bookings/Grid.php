...
class Grid
{
	...
	public function render_footer()
	{
		...
		$multiple_label = lang('multiple_label');
		$input_label = form_label($input . $multiple_label, 'multi_select', ['class' => 'ni', 'style' => 'display: inline-block;margin-bottom:8px']);

		$check_block = "<div class='block b-50' style='text-align:right'>{$input_label}</div>";

		$submit_button = sprintf(
			"<button type='submit' style='margin-left:32px;display:none;' class='multi-select-content'>%s</button>",
			lang('create_bookings')
		);
